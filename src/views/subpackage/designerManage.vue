    ...
    <div class="box-content">
        ...
        <el-table :data="tableLsit" :height="tableHeight" border>
          ...
          >
            <template slot-scope="scope">
              <el-image
                class="img-style"
                :preview-src-list="[scope.row.headImg[0].img]"
                :src="scope.row.headImg[0].img"
                fit="fill"
              ></el-image>
            </template>
          </el-table-column>
      ...
      >
        <el-form
          ...
          class="demo-ruleForm"
        >
          <el-form-item label="设计师头像" prop="name">
            <el-input
              maxlength="20"
              class="input-width"
              placeholder="请输入设计师头像"
...
export default {
  ...
  methods: {
      ...
      getTablePage({
        ...
        .then((res) => {
          ...
          this.tableLsit.map((item) => {
            return (item.headImg = item.headImg
              ? JSON.parse(item.headImg)
              : [{ img: "" }]);
          });
    ...
    async findDtails(row) {
      ...
      try {
        const imgObj = JSON.parse(data.headImg)[0];
    ...
    submitForm() {
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          if (!this.imgArr.length) return showToast("请上传头像", "error");
          this.btnloading = true;
          this.form.headImg = JSON.stringify(this.imgArr);
          addRowIn(this.form)
            .then(() => {
              this.imgId = "";
              this.btnloading = false;
